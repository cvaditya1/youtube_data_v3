...
class YtChannel {
  ...
  _extractChannelUploadsPlaylistId() {
    ...
    ChannelContentDetailsRelatedPlaylists channelPlaylists =
        contentDetails?.relatedPlaylists ?? null;
    uploadsId = channelPlaylists?.uploads ?? null;
  }

  //ToDo: Required Limits for max result
  Future<List<YtVideo>> getPageOfVideos(
      {int numResults,
      bool showDuration,
      bool showNextPage,
      bool showPrevPage}) async {
    ...
    String pageToken = (!getNextPage && !getPreviousPage)
        ? null
        : (getNextPage
            ? _nextPageToken
            : (getPreviousPage ? _previousPageToken : null));

    PlaylistItemListResponse response = await _ytApi.playlistItems.list(part,
        playlistId: uploadsId, maxResults: numResults, pageToken: pageToken);
  ...
  Future<List<YtVideo>> _getNewVideoList(
      PlaylistItemListResponse response, bool showDuration) async {
  ...
  Future _fetchVideoDetails(List<YtVideo> videoList) async {
    ...
    if (videoIdList.length > 0) {
      ...
      VideoListResponse response =
          await _ytApi.videos.list(part, id: ids, maxResults: videoList.length);
      List<Video> videoListResp = response.items;

      await Future.forEach(videoListResp, (Video video) {
        YtVideo ytVideo = videoList.singleWhere(
            (ytVideo) => ytVideo.videoId == video.id,
            orElse: () => null);
        ytVideo.duration =
            _getDuration(video?.contentDetails?.duration ?? "") ?? "";
      });
    }
  }
